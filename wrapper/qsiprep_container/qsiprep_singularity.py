#!/usr/bin/env python
"""
Using qsipost within a singularity container (qsipost-<version>.simg)


To run qsipost with singularity, a singularity image must be built (see Installation).
...
__longdesc__ = """\
This package provides an intuitive interface for running the qsipost
...
def get_parser():
    ...
    parser.add_argument(
        ...
        metavar="IMG",
        type=str,
        default=os.path.expanduser("~") + "/qsipost-latest.simg",
        help="image name",
    )
...
def main():
    ...
    if check < 1:
        ...
        if opts.help:
            ...
            print("qsipost: Could not find singularity command... Is it installed?")
        return 1
    ...
    if opts.recon_input:
        command.extend(["-B", ":".join((opts.recon_input, "/sngl/qsipost-output"))])
        main_args.extend(["--recon-input", "/sngl/qsipost-output"])
    if opts.recon_spec:
