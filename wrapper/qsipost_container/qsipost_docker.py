#!/usr/bin/env python
"""
The qsipost on Docker wrapper


This is a lightweight Python wrapper to run qsipost.
...
__version__ = "latest"
__packagename__ = "qsipost-container"
__author__ = ""
...
__longdesc__ = """\
This package is a basic wrapper for qsipost that generates the appropriate
Docker commands, providing an intuitive interface to running the qsipost
...
PKG_PATH = "/opt/conda/envs/qsipost/lib/python3.10/site-packages"
...
def get_parser():
    ...
    g_dev = parser.add_argument_group(
        "Developer options", "Tools for testing and debugging qsipost"
    )
    g_dev.add_argument(
        "-f",
        "--patch-qsipost",
        metavar="PATH",
        type=os.path.abspath,
        help="working qsipost repository",
    )
    g_dev.add_argument(
        "-p",
        "--patch-nipype",
    ...
    g_dev.add_argument(
        "--shell", action="store_true", help="open shell in image instead of running qsipost"
    )
    g_dev.add_argument(
        "--config",
        metavar="PATH",
...
def main():
    ...
    if check < 1:
        if opts.version:
            print("qsipost wrapper {!s}".format(__version__))
        if opts.help:
            parser.print_help()
        if check == -1:
            print("qsipost: Could not find docker command... Is it installed?")
        else:
            print("qsipost: Make sure you have permission to run 'docker'")
        return 1
    ...
    if not (opts.help or opts.version or "--reports-only" in unknown_args) and mem_total < 8000:
        print(
            ...
            "environment.\nSome parts of qsipost may fail to complete."
        )
    ...
    if opts.recon_input:
        command.extend(["-v", ":".join((opts.recon_input, "/qsipost-output", "ro"))])
        main_args.extend(["--recon-input", "/qsipost-output"])
    ...
    ret = subprocess.run(command)
    if ret.returncode:
        print("qsipost: Please report errors to {}".format(__bugreports__))
    return ret.returncode
