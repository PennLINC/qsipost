...
You can send the outputs from ``qsipost`` to other software packages
...
`github <https://github.com/pennlinc/qsipost/tree/main/qsipost/data/pipelines>`_. For extra
...
``--recon-spec`` argument. For example::
  ...
  $ qsipost-docker \
      ...
      --recon_input /output/from/qsipost \
      --recon_spec dsi_studio_gqi \
      --fs-license-file /path/to/license.txt


``qsipost`` supports a limited number of algorithms that are wrapped in
...
QSIPost. Instead of preprocessing these from scratch, you can run reconstruction
...
Data preprocessed by ``qsipost`` may be missing a preprocessed T1w image if the ``--dwi-only`` flag
...
and from ``qsipost``::
    ...
    derivatives/qsipost/sub-x
    derivatives/qsipost/sub-y
    derivatives/qsipost/sub-z

You can run::

  $ qsipost-docker \
      derivatives/qsipost derivatives participant \
      --recon_input derivatives/qsipost \
      --recon_spec mrtrix_multishell_msmt_ACT-hsvs \
      --freesurfer-input derivatives/freesurfer \
      --fs-license-file /path/to/license.txt

This will read the FreeSurfer data, align it to the ``qsipost`` results and use it
...
If ``qsipost`` performed anatomical preprocessing
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In most cases human MRI experiments include a T1-weighted anatomical image.
By default ``qsipost`` performs some processing steps on this image,
...
``qsipost``, the ``brain.mgz`` image from freesurfer is registered to the
AC-PC and DWI-aligned ``desc-preproc_T1w.nii`` image in the ``qsipost`` outputs.
The transform from freesurfer native space into alignment with the ``qsipost``
...
``qsipost`` results. Instead the DWI images have been aligned to AC-PC
...
The source of the brain mask depends on available data and user options.
  ...
  * If ``qsipost`` ran normally and the reconstruction workflows are run
    without ``--dwi-only``, the brain mask estimated by ``antsBrainExtraction``
    during preprocessing is used.
  * If no T1w data is available in the ``qsipost`` outputs and the user
    supplies FreeSurfer data with ``--freesurfer-input``, the brain mask
    created by FreeSurfer is used.
  * If you specify ``--dwi-only`` when ``qsipost`` performs the reconstruction
    OR if no preprocessed T1w images (either via ``qsipost`` outputs or
...
qsipost-generated _dwiref.nii.gz and preproc_T1w.nii.gz volumes; .tck files can be visualized
...
Reorients the ``qsipost`` preprocessed DWI and bval/bvec to the standard FSL orientation.
This can be useful if FSL tools will be applied outside of ``qsipost``.
...
come packaged with ``qsipost`` in the Docker image. In this case, the
...
outputs from ``qsipost`` will be its inputs.
...
output from ``qsipost`` by adding ``"action": "reconstruction"``. Additional
...
``qsipost-container`` you can execute this pipeline with::
...
``qsipost`` will construct as many connectivity matrices as it can given the reconstruction
...
The following atlases are included in ``qsipost`` and are used by default in the
...
It's possible to use your own atlases provided you can match the format ``qsipost`` uses to
read atlases. The ``qsipost`` atlas set can be downloaded directly from
...
the container at ``/atlas/qsirecon_atlases``. If using ``qsipost-docker`` or
``qsipost-singularity`` this can be done with ``--custom-atlases /path/to/my/atlases`` or
...
`MNI152NLin2009cAsym <https://github.com/pennlinc/qsipost/blob/main/qsipost/data/mni_1mm_t1w_lps.nii.gz>`_
included in ``qsipost``.
