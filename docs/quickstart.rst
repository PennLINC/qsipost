...
There are many options for running ``qsipost`` but most have sensible defaults and
...
One way to process these data would be to call ``qsipost`` like this::
...
``qsipost`` will infer that the dwi scans are in the same **warped space** - that their
...
``derivitaves/qsipost/sub-1/ses-1/dwi/sub-1_ses-1_acq-multishell_desc-preproc_dwi.nii.gz``. otherwise
...
QSIPost will use BSpline interpolation instead of Lanczos windowed sinc
interpolation.
...
The image processing operations performed by QSIPost are configured by default
...
While QSIPost runs with `-v -v`, you will see lots of unintuitive output
in the terminal like::

  [Node] Setting-up "qsipost_wf.single_subject_PNC_wf.dwi_finalize_acq_realistic_wf.transform_dwis_t1.final_b0_ref.b0ref_reportlet" in "/scratch/qsipost_wf/single_subject_PNC_wf/dwi_finalize_acq_realistic_wf/transform_dwis_t1/final_b0_ref/b0ref_reportlet".
    ...
    201229-21:33:48,51 nipype.workflow INFO:
      [MultiProc] Running 2 tasks, and 3 jobs ready. Free memory (GB): 3.70/4.00, Free processors: 0/2.
                        Currently running:
                          * qsipost_wf.single_subject_PNC_wf.dwi_finalize_acq_realistic_wf.transform_dwis_t1.final_b0_ref.b0ref_reportlet
                          * qsipost_wf.single_subject_PNC_wf.anat_preproc_wf.mni_mask
...
Also in this example you can see that QSIPost was run with ``--nthreads 2``
