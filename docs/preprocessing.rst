...
QSIPost builds a pipeline based on your BIDS inputs. In general the pipeline will incorporate
...
you can call qsipost more than once with BIDS filters to process the different scans.
...
BIDS filters allow users to filter the set of images available to QSIPost at run
time. BIDS filters should be stored in a json file and passed to QSIPost with
...
of the file you will send to ``--bids-filter-file``. The queries in QSIPost are::
...
QSIPost. Each of these image processing operations has assumptions about its
...
QSIPost can be configured to produce a very similar pipeline to the HCP dMRI pipelines.
...
output from ``qsipost`` that is directly comparable to the HCP dMRI pipeline you
...
qsipost generates three broad classes of outcomes:
...
qsipost outputs summary reports, written to ``<output
dir>/qsipost/sub-<subject_label>.html``. These reports provide a quick way to
...
Preprocessed data (qsipost *derivatives*)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

There are additional files, called "Derivatives", written to
``<output dir>/qsipost/sub-<subject_label>/``.
...
See implementation on :mod:`~qsipost.workflows.dwi.confounds.init_dwi_confs_wf`.


For each DWI processed by qsipost, a
``<output_folder>/qsipost/sub-<sub_id>/func/sub-<sub_id>_task-<task_id>_run-<run_id>_confounds.tsv``
...
``qsipost`` adapts its pipeline depending on what data and metadata are
...
:mod:`qsipost.workflows.anatomical.init_anat_preproc_wf`

.. workflow::
    :graph2use: orig
    :simple_form: yes

    from qsipost.workflows.anatomical import init_anat_preproc_wf
...
As of version 0.18 QSIPost has been changed to be very flexible with anatomical
...
As a dMRI-focused tool, QSIPost only uses an *anatomical reference* image for an
...
reference scans. QSIPost can use either T1-weighted ot T2-weighted 3D images as
...
Therefore, in the case of three or more images, ``qsipost`` constructs
...
QSIPost call. This will swap the default MNI152NLin2009cAsym template
...
:mod:`qsipost.workflows.dwi.base.init_dwi_preproc_wf`

.. workflow::
    :graph2use: orig
    :simple_form: yes

    from qsipost.workflows.dwi.base import init_dwi_preproc_wf
...
:mod:`qsipost.workflows.dwi.fsl.init_fsl_hmc_wf`
...
there will be *two* total interpolations in the FSL-based qsipost workflow, as
...
.. workflow::
    ...
    from qsipost.workflows.dwi.fsl import init_fsl_hmc_wf
...
:mod:`qsipost.workflows.dwi.hmc.init_dwi_hmc_wf`
...
``qsipost`` introduces a method for head motion correction that iteratively
...
.. workflow::
    ...
    from qsipost.workflows.dwi.hmc_sdc import init_qsipost_hmcsdc_wf
    wf = init_qsipost_hmcsdc_wf({'dwi_series':['dwi1.nii', 'dwi2.nii'],
                                'fieldmap_info': {'suffix': None},
                                'dwi_series_pedir': 'j'},
                                source_file='/data/sub-1/dwi/sub-1_dwi.nii.gz',
                                b0_threshold=100,
...
:mod:`qsipost.workflows.fieldmap.base.init_sdc_wf`

.. figure:: _static/unwarping.svg

The are three kinds of SDC available in qsipost:
  ...
  3. :ref:`sdc_fieldmapless`: The SyN-based susceptibility distortion correction
     ...
     calling qsipost. Briefly, this method estimates a SDC warp using ANTS SyN based
     on an average fieldmap in MNI space. For details on this method.

``qsipost`` determines if a fieldmap should be used based on the ``"IntendedFor"``
...
In QSIPost versions up until 0.13, up to 3 b=0 images were selected per
warped group as input to ``TOPUP``. The images were selected to be
evenly spaced within their acquisitions.

In versions 0.13 and later, QSIPost finds the "most representative" b=0
...
It is possible to use QSIPost to process *only* diffusion-weighted images. In
...
:mod:`qsipost.workflows.dwi.util.init_dwi_reference_wf`

.. workflow::
    :graph2use: orig
    :simple_form: yes

    from qsipost.workflows.anatomical import init_dwi_reference_wf
...
:mod:`qsipost.workflows.dwi.resampling.init_dwi_trans_wf`

.. workflow::
    :graph2use: orig
    :simple_form: yes

    from qsipost.workflows.dwi.resampling import init_dwi_trans_wf
...
:mod:`qsipost.workflows.dwi.registration.init_b0_to_anat_registration_wf`

.. workflow::
    :graph2use: orig
    :simple_form: yes

    from qsipost.workflows.dwi.registration import init_b0_to_anat_registration_wf
