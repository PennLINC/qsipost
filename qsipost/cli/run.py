...
#
# Changed to run qsipost/qsirecon
#
...
def main():
    ...
    if "pdb" in config.execution.debug:
        from qsipost.utils.debug import setup_exceptionhook
    ...
    qsipost_wf = retval.get("workflow", None)
    exec_mode = retval.get("exec_mode", "QSIPost")
    output_dir = (
        config.execution.qsipost_dir if exec_mode == "QSIPost" else config.execution.output_dir
    )
    ...
    exitcode = exitcode or (qsipost_wf is None) * EX_SOFTWARE
    if exitcode != 0:
        sys.exit(exitcode)
    ...
    with Manager() as mgr:
        ...
        p = Process(target=build_boilerplate, args=(str(config_file), qsipost_wf))
        p.start()
        p.join()
    ...
    try:
        ...
        if sentry_sdk is not None:
            success_message = "QSIPost finished without errors"
        ...
        if boiler_file.exists():
            ...
            config.loggers.workflow.log(
                25,
                "Works derived from this QSIPost execution should include the "
        ...
        write_bidsignore(config.execution.qsipost_dir)
    ...
    try:
        ...
        if not config.execution.notrack:
            ...
            crashfolders = [
                config.execution.qsipost_dir / f"sub-{s}" / "log" / config.execution.run_uuid
                for s in config.execution.participant_label
            ]
            for crashfolder in crashfolders:
                for crashfile in crashfolder.glob("crash*.*"):
            ...
            if sentry_sdk is not None and "Workflow did not execute cleanly" not in str(e):
                sentry_sdk.capture_exception(e)
        config.loggers.workflow.critical("QSIPost failed: %s", e)
        raise
    else:
        config.loggers.workflow.log(25, "QSIPost finished successfully!")
        if sentry_sdk is not None:
