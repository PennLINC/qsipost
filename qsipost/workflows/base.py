...
from qsipost.engine import Workflow


def init_qsipost_wf():
    """
    This workflow organizes the execution of QSIPost, with a sub-workflow for
    each subject.
    ...
    qsipost_wf = Workflow(name=f"qsipost_{ver.major}_{ver.minor}_wf")
    qsipost_wf.base_dir = config.execution.work_dir

    if config.workflow.recon_input_pipeline not in ("qsipost", "ukb"):
        raise NotImplementedError(
            f"{config.workflow.recon_input_pipeline} is not supported as recon-input yet."
        )

    if config.workflow.recon_input_pipeline == "qsipost":
        ...
        from qsipost.utils.ingress import collect_ukb_participants, create_ukb_layout
    ...
    for subject_id in to_recon_list:
        ...
        single_subject_wf.config["execution"]["crashdump_dir"] = str(
            config.execution.qsipost_dir / f"sub-{subject_id}" / "log" / config.execution.run_uuid
        )
...
def init_single_subject_recon_wf(subject_id):
    ...
    """
    from qsipost.interfaces.ingress import QsiReconDWIIngress, UKBioBankDWIIngress
    from qsipost.interfaces.interchange import (
        ReconWorkflowInputs,
        anatomical_workflow_outputs,
        qsipost_output_names,
        recon_workflow_anatomical_input_fields,
        recon_workflow_input_fields,
    )
    from qsipost.workflows.recon.anatomical import (
        init_dwi_recon_anatomical_workflow,
        init_highres_recon_anatomical_wf,
    )
    from qsipost.workflows.recon.build_workflow import init_dwi_recon_workflow
...
Many internal operations of *qsipost* use
*Nilearn* {nilearn_ver} [@nilearn, RRID:SCR_001362] and
*Dipy* {dipy_ver}[@dipy].
For more details of the pipeline, see [the section corresponding
to workflows in *qsipost*'s documentation]\
(https://qsipost.readthedocs.io/en/latest/workflows.html \
"qsipost's documentation").
...
### References
    ...
    # This is here because qsipost currently only makes one anatomical result per subject
    ...
    for dwi_input in dwi_recon_inputs:
        ...
        workflow.connect([
            ...
            (dwi_ingress_nodes[dwi_file], dwi_individual_anatomical_wfs[dwi_file],
             [(trait, "inputnode." + trait) for trait in qsipost_output_names]),
            ...
            (anat_ingress_node if config.workflow.recon_input_pipeline == "qsipost" \
...
def _load_recon_spec():
    ...
    prepackaged_dir = pkgrf("qsipost", "data/pipelines")
...
def _get_iterable_dwi_inputs(subject_id):
    ...
    If qsipost was used as the pipeline source, the iterable is going to be the
    ...
    if config.workflow.recon_input_pipeline == "qsipost":
        # If recon_input is specified without qsipost, check if we can find the subject dir
        if not (recon_input_directory / f"sub-{subject_id}").exists():
            config.loggers.workflow.info(
                "%s not in %s, trying recon_input=%s",
                subject_id,
                recon_input_directory,
                recon_input_directory / "qsipost",
            )

            recon_input_directory = recon_input_directory / "qsipost"
            if not (recon_input_directory / f"sub-{subject_id}").exists():
                raise Exception(
