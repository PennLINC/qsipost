...
ABOUT_TEMPLATE = """\t<ul>
\t\t<li>qsipost version: {version}</li>
\t\t<li>qsipost command: <code>{command}</code></li>
    ...
    def _generate_segment(self):
        if not isdefined(self.inputs.subjects_dir):
            ...
            if recon.cmdline.startswith("echo"):
                ...
                freesurfer_status = "Run by qsipost"
...
class AboutSummaryInputSpec(BaseInterfaceInputSpec):
    version = Str(desc="qsipost version")
    command = Str(desc="qsipost command")
