...
#
# Changed to build qsipost and qsirecon workflows
#
...
``multiprocessing.Process`` that allows qsipost to enforce
...
def build_workflow(config_file, exec_mode, retval):
    ...
    if notice_path.exists():
        ...
        banner += [f"QSIPost {version}"]
        banner += notice_path.read_text().splitlines(keepends=False)[1:]
        banner += ["#" * len(banner[1])]
    build_log.log(25, f"\n{' ' * 9}".join(banner))

    # warn if older results exist: check for dataset_description.json in output folder
    # msg = check_pipeline_version("QSIPost", version, qsipost_dir / "dataset_description.json")
    # if msg is not None:
    ...
    if not config.execution.fs_license_file.exists():
        build_log.critical(
            """\
ERROR: a valid license file is required for FreeSurfer to run. QSIPost looked for an existing \
...
def build_boilerplate(config_file, workflow):
    ...
    config.load(config_file)
    logs_path = config.execution.qsipost_dir / "logs"
    ...
    if citation_files["md"].exists():
        ...
        bib_text = Path(pkgrf("qsipost", "data/boilerplate.bib")).read_text()
        citation_files["bib"].write_text(
            bib_text.replace("QSIPost <version>", f"QSIPost {config.environment.version}")
        )
        ...
        cmd = [
            ...
            "pandoc-citeproc",
            "--metadata",
            'pagetitle="QSIPost citation boilerplate"',
