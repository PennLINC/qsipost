...
Note that, even if ``--force-syn`` is given, the functional outputs of qsipost will
...
def init_syn_sdc_wf(bold_pe=None, atlas_threshold=2):
    ...
    .. workflow ::
        ...
        from qsipost.workflows.fieldmap.syn import init_syn_sdc_wf
        wf = init_syn_sdc_wf(
            bold_pe='j',
            omp_nthreads=8)
...
"""
    ...
    atlas_img = pkgr.resource_filename("qsipost", "data/mni_lps_fmap_atlas.nii.gz")
    # Registration specifications
    affine_transform = pkgr.resource_filename("qsipost", "data/affine.json")
    syn_transform = pkgr.resource_filename("qsipost", "data/susceptibility_syn.json")
