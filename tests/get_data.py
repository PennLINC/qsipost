...
from qsipost.cli.run import main as cli_main
...
RECON_INPUT = op.join(WORKING_DIR, "output/qsipost")
...
def link_anatomicals(new_dir):
    old_wd = op.join(WORKING_DIR, "qsipost_wf")
    new_wd = op.join(new_dir, "qsipost_wf")
