import pytest
import os
from qsipost.cli.run import set_freesurfer_license, validate_bids, get_parser
...
def test_required_recononly(monkeypatch):
    ...
    monkeypatch.setattr('qsipost.cli.run.sys.argv', args)
...
def test_validate_bids(monkeypatch, opts_str, will_validate):
    ...
    monkeypatch.setattr("qsipost.utils.bids.validate_input_dir", lambda *kargs: True)
